...
if [ -z "$FAH_PROCESS_ID" ]; then
  ...
  _WRITE_PROCESS_CONTEXT $FAH_PROCESS_ID "REPROCESS_SCOPE" "OMNI_HIT"
  if [ $? -ne 0 ]; then
    _LOG "Error writing process context. Installation FAILED."
    $PLAT_HOME/tools/metadata/delete_process.sh "$FAH_PROCESS_NAME"
    exit 1
...
if [ $? -ne 0 ]; then
  ...
  _LOG "Setting up Parameters for Fact"
  _WRITE_PROCESS_CONTEXT $FACT_PROCESS_ID "SUP_MAP_TABLE" "ETLADS_HCOM_SUPPLIER_PROPERTY_MAP"
  _WRITE_PROCESS_CONTEXT $FACT_PROCESS_ID "SRC_BOOKMARK_OMNI" "2013-10-01 00"
  _WRITE_PROCESS_CONTEXT $FACT_PROCESS_ID "SRC_BOOKMARK_BKG" "2013-10-01"
  _WRITE_PROCESS_CONTEXT $FACT_PROCESS_ID "STEP_TO_PROCESS_FROM" "$STEP_LOAD_STAGING_DATA"
