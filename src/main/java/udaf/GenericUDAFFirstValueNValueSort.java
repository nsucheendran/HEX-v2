...
/**
 * This is a UDAF that returns the first column value appearing in the sorted rows of a group. The rows are sorted by the column values
 * provided along. Any no. of sort column values can be provided.
 * 
 * Input values: 1. Column value to be returned 2. Variable no. of sort column values
 * 
...
public class GenericUDAFFirstValueNValueSort extends AbstractGenericUDAFResolver {
    ...
    public GenericUDAFEvaluator getEvaluator(TypeInfo[] parameters) throws SemanticException {
        if (parameters.length < 2) {
            throw new UDFArgumentTypeException(2, "Min 2 arguments expected: 1). colm value to be returned, "
        ...
        for (int num = 1; num < parameters.length; num++) {
            if (parameters[num].getCategory() != ObjectInspector.Category.PRIMITIVE) {
                throw new UDFArgumentTypeException(num, "Only primitive type arguments are accepted in orderby colms but "
    ...
    private static class MinOrderedSet implements AggregationBuffer {
        ...
        private boolean areNewSortKeysLesser(Object[] newKeys, ObjectInspector[] sortKeyOI) {
    ...
    public static class GenericUDAFOrderedNValueEvaluator extends GenericUDAFEvaluator {
        ...
        public ObjectInspector init(Mode m, ObjectInspector[] parameters) throws HiveException {
            ...
            if (m.equals(Mode.COMPLETE) || m.equals(Mode.PARTIAL1)) {
                ...
                StructObjectInspector soi = (StructObjectInspector) mapOI.getMapValueObjectInspector();
                List<? extends StructField> sortKeysListStruct = soi.getAllStructFieldRefs();
                stdSortKeysOI = new ObjectInspector[sortKeysListStruct.size()];
                for (int i = 0; i < sortKeysListStruct.size(); i++) {
                    stdSortKeysOI[i] = ObjectInspectorUtils.getStandardObjectInspector(sortKeysListStruct.get(i).getFieldObjectInspector());
                }
                writableValueOI = ObjectInspectorUtils.getWritableObjectInspector(mapOI.getMapKeyObjectInspector());
            }
            ...
            if (m == Mode.PARTIAL1) {
                ...
                return ObjectInspectorUtils.getWritableObjectInspector(ObjectInspectorFactory.getStandardMapObjectInspector(
                        writableValueOI,
        ...
        public void iterate(AggregationBuffer agg, Object[] parameters) throws HiveException {
            ...
            Object value = ObjectInspectorUtils.copyToStandardObject(parameters[0], valueOI, ObjectInspectorCopyOption.WRITABLE);
            Object[] stdSortKeys = new Object[parameters.length - 1];
            for (int num = 0; num < sortKeysOI.length; num++) {
                if (parameters[num + 1] != null) {
                    Object key = ObjectInspectorUtils.copyToStandardObject(parameters[num + 1], sortKeysOI[num],
            ...
            // one
            existingRecord.checkAndSet(stdSortKeys, stdSortKeysOI, value);
        }
            ...
            if (partial != null) {
                ...
                for (Map.Entry<Object, Object> data : partialData.entrySet()) {
                    ...
                    for (int i = 0; i < sortKeys.size(); i++) {
                        stdSortKeys[i] = ObjectInspectorUtils.copyToStandardObject(sortKeys.get(i), stdSortKeysOI[i],
                                ObjectInspectorCopyOption.WRITABLE);
                    }
                    Object newVal = ObjectInspectorUtils.copyToStandardObject(data.getKey(), writableValueOI);
                    myagg.checkAndSet(stdSortKeys, stdSortKeysOI, newVal);
                }
            }
        }
