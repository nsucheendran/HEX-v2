    ...
    public GenericUDAFEvaluator getEvaluator(TypeInfo[] parameters) throws SemanticException {
        ...
        for (int num = 1; num < parameters.length; num++) {
            if (parameters[num].getCategory() != ObjectInspector.Category.PRIMITIVE) {
                throw new UDFArgumentTypeException(num, "Only primitive type arguments are accepted in orderby colms but " + parameters[num].getTypeName()
                        + " was passed as parameter " + num);
            }
            switch (((PrimitiveTypeInfo) parameters[num]).getPrimitiveCategory()) {
            case VOID:
            case UNKNOWN:
                throw new UDFArgumentTypeException(0, ((PrimitiveTypeInfo) parameters[num]).getPrimitiveCategory() + " arguments not acceptable in order by columns");
            default:
                break;
            }
        }
    ...
    public static class GenericUDAFOrderedNValueEvaluator extends GenericUDAFEvaluator {
        ...
        public ObjectInspector init(Mode m, ObjectInspector[] parameters) throws HiveException {
            ...
            if (m == Mode.PARTIAL1 || m == Mode.PARTIAL2) {
                ObjectInspector mapOI = ObjectInspectorFactory.getStandardMapObjectInspector(PrimitiveObjectInspectorFactory.writableStringObjectInspector, writableValueOI);
                ...
                // is returned
                ObjectInspector writableValueOI = ObjectInspectorUtils.getWritableObjectInspector(mapOI.getMapValueObjectInspector());
        ...
        public void iterate(AggregationBuffer agg, Object[] parameters) throws HiveException {
            ...
            for (int num = 0; num < sortKeyOI.length; num++) {
                if (parameters[num + 1] != null) {
                    Object key = ObjectInspectorUtils.copyToStandardObject(parameters[num + 1], sortKeyOI[num], ObjectInspectorCopyOption.WRITABLE);
                    sortKey.append(key);
                }
            }
