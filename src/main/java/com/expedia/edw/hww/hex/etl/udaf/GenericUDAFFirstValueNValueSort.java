...
/**
 * This is a UDAF that returns the first column value appearing in the sorted rows of a group. The rows are sorted by
 * the column values provided along. Any no. of sort column values can be provided. Input values: 1. Column value to be
 * returned 2. Variable no. of sort column values Output value: Returns column value for the 1st row in the sorted data
 * rows.
 */
public class GenericUDAFFirstValueNValueSort extends AbstractGenericUDAFResolver {
  // private static final Log LOG =
  // LogFactory.getLog(GenericUDAFFirstValueNValueSort.class.getName());
    ...
    public ObjectInspector init(Mode m, ObjectInspector[] parameters) throws HiveException {
      ...
      if (m.equals(Mode.COMPLETE) || m.equals(Mode.PARTIAL1)) {
        ...
        for (int i = 0; i < sortKeysListStruct.size(); i++) {
          stdSortKeysOI[i] = ObjectInspectorUtils.getStandardObjectInspector(sortKeysListStruct
              .get(i)
