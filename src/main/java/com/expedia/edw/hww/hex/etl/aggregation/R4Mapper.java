...
public class R4Mapper extends Mapper<BytesWritable, Text, TextMultiple, TextMultiple> {
  ...
  /*
   * apply rtable (smaller table) as a filter and emit non-null rtable rows iff the lrow passes the join criteria
   * (technically not inner/right-outer join, but practically an inner join where the join key combinations are assumed
   * to be unique in rtable)
   */
