...
public class GenericUDFRandomizeInput extends GenericUDF {
	...
	public ObjectInspector initialize(ObjectInspector[] arguments)
		...
		if (arguments.length == 5) {
			if (arguments[4] != null
					&& !(arguments[4] instanceof ListObjectInspector)) {
				throw new UDFArgumentException(
						"fifth argument must be a list / array");
			}
			this.whitelistValuesOI = (ListObjectInspector) arguments[4];
		}
		if (!(inputValOI instanceof PrimitiveObjectInspector)) {
			throw new UDFArgumentException(
					"first argument must be a primitive data type");
		}
		...
		if (!(seedValSeparatorOI instanceof StringObjectInspector)) {
			throw new UDFArgumentException(
					"third argument must be of type String");
		}
		if (randomizeOI != null
				&& !(randomizeOI instanceof BooleanObjectInspector)) {
			throw new UDFArgumentException(
					"fourth argument must be of type Boolean");
		}
	...
	public Object evaluate(DeferredObject[] arguments) throws HiveException {
		String inputVal = (String) ObjectInspectorUtils
				.copyToStandardJavaObject(
						arguments[0].get(),
						PrimitiveObjectInspectorFactory.javaStringObjectInspector);
		Integer seed = (Integer) ObjectInspectorUtils.copyToStandardJavaObject(
				arguments[1].get(),
				PrimitiveObjectInspectorFactory.writableIntObjectInspector);
		String seedValSeparator = (String) ObjectInspectorUtils
				.copyToStandardJavaObject(
						arguments[2].get(),
						PrimitiveObjectInspectorFactory.javaStringObjectInspector);
		Boolean randomizeInput = null;
		if (arguments.length >= 4) {
			randomizeInput = (Boolean) ObjectInspectorUtils
					.copyToStandardJavaObject(
							arguments[3].get(),
		...
		if (whitelistValuesOI != null) {
			whitelist = (List<String>) this.whitelistValuesOI
					.getList(arguments[4].get());
		}
		// check for nulls
		...
		if (randomizeInput == null || randomizeInput) {
			...
			return Lists.newArrayList(new Text(inputValText + seedValSeparator
					+ randomVal));
		}
	...
	public String getDisplayString(String[] children) {
		...
		return "randomize(" + children[0] + ", " + children[1] + ", "
