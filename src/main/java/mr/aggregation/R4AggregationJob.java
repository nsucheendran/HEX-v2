...
public final class R4AggregationJob extends Configured implements Tool {
	...
	public int run(final String[] arg0) throws IOException, TException,
		...
		try {
			...
			List<FieldSchema> fieldschemas = cl.getFields(sourceDbName,
					sourceTableName);
			lhsfields = new ArrayList<String>(fieldschemas.size());
			for (FieldSchema field : fieldschemas) {
				lhsfields.add(field.getName());
			}
			List<FieldSchema> rhsfieldschemas = cl.getFields(targetDbName,
					reportTableName);
		...
		configurator.lhsFields(lhsfields).rhsFields(rhsfields)
				.numReduceTasks(numReduceTasks);
		configurator.configureJob(job);
		job.getConfiguration().set(
				"data",
				getReportDataAsString(reportTableName, targetDbName,
						configurator, job, cl));
		// job.getConfiguration().set
	...
	private String getReportDataAsString(String reportTableName,
			String reportDbName, JobConfigurator configurator, Job job,
