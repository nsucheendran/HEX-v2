...
public final class R4AggregationJob extends Configured implements Tool {
  ...
  private Set<String> getPartitions(final String tmpOutputPath, final String tableOutputPath, final Job job) throws IOException {
    ...
    while (files.hasNext()) {
      ...
      if (found) {
        ...
        if (partitionNames.add(partition)) {
          ...
          String tmpPartitionPathLoc = new StringBuilder(m.group(1)).append(m.group(2)).append(m.group(3)).append(m.group(4)).toString();
