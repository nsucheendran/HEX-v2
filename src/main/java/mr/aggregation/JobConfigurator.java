...
public final class JobConfigurator {
	...
	public JobConfigurator rhsFields(List<String> rhsFields) {
		...
		for (String field : rhsFields) {
			if (equiJoinKeys.values().contains(field)
					|| lteJoinKeys.values().contains(field)
					|| gteJoinKeys.values().contains(field)
	...
	private StringBuilder configString(Map<String, Integer> equiLhsPosMap,
			Map<String, String> joinKeys) {
		int rk = 0;
		...
		for (Map.Entry<String, String> entry : joinKeys.entrySet()) {
			...
			if (equiLhsPosMap.containsKey(fieldName)) {
				...
				equiJoinPosMap.append(equiLhsPosMap.get(fieldName)).append("=")
						.append(rhsPosMap.get(rField).two);
			}
		}
