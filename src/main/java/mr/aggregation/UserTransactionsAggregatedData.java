...
public class UserTransactionsAggregatedData {
	...
	public void addTransaction(UserTransactionData newTransData) {
		...
		AggItineraryData aggTransByItinNumber = allTransDataByItinNumber
				.get(newTransData.getItinNumber());
		...
		if (newTransData.getNumTrans() != 0) {
			...
			allTransDataByItinNumber.put(newTransData.getItinNumber(),
					aggTransByItinNumber);
		}
	...
	public boolean isRepeatPurchaser() {
		...
		for (Map.Entry<String, AggItineraryData> aggTransByItinNumber : allTransDataByItinNumber
				.entrySet()) {
			if (aggTransByItinNumber.getValue().getNumTrans() > 0) {
				if (firstPurchaseDate == null
						&& StringUtils.isNotEmpty(aggTransByItinNumber
								.getValue().getMinTransDate())) {
					firstPurchaseDate = aggTransByItinNumber.getValue()
							.getMinTransDate();
				} else if (!aggTransByItinNumber.getValue().getMinTransDate()
						.equals(firstPurchaseDate)) {
	...
	static class AggItineraryData {
		...
		public void setMinTransDate(String transDate) {
			if (this.minTransDate == null
					|| transDate.compareTo(minTransDate) < 0) {
