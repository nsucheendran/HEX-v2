...
public class R4Reducer extends
		Reducer<TextMultiple, TextMultiple, BytesWritable, Text> {
	...
	public final void reduce(final TextMultiple key,
		...
		perUserTransactionData = new HashMap<String, UserTransactionsAggregatedData>(
				5000);
		...
		/*
		 * guid => 0 itin_number => 1 trans_date => 2 num_transactions => 3
		 * bkg_gbv => 4 bkg_room_nights => 5 omniture_gbv => 6
		...
		for (TextMultiple value : values) {
			...
			UserTransactionsAggregatedData userAggTransData = perUserTransactionData
					.get(guid.toString());
		...
		for (Map.Entry<String, UserTransactionsAggregatedData> userAggTransData : perUserTransactionData
				.entrySet()) {
			...
			if ((userAggTransData.getValue().isPurchaser() || userAggTransData
					.getValue().isCanceller())
					&& userAggTransData.getValue().getNetTransactions() <= 0) {
				numNilNetOrdersPurchasers++;
			}
			numCancellations += userAggTransData.getValue()
					.getNumCancellations();
			netOrders += userAggTransData.getValue().getNetTransactions();
			netBkgGBV += userAggTransData.getValue().getTotalBkgGbv();
			netBkgRoomNights += userAggTransData.getValue()
					.getTotalBkgRoomNights();
			netOmnitureGBV += userAggTransData.getValue().getTotalOmnitureGbv();
			netOmnitureRoomNights += userAggTransData.getValue()
					.getTotalOmnitureRoomNights();
			netGrossProfit += userAggTransData.getValue().getNetGrossProfit();
		}
		outStr.setLength(0);
		key.toStringBuilder(
				outStr.append(numUniqueViewers).append(SEP)
						...
						.append(netBkgRoomNights).append(SEP)
						.append(netOmnitureGBV).append(SEP)
						.append(netOmnitureRoomNights).append(SEP)
						.append(netGrossProfit).append(SEP)
						.append(numRepeatPurchasers).append(SEP), excludes);
		outStr.append(SEP).append(key.getTextElementAt(3).toString())
		...
		// mos.write("outroot", bw, outText,
		// generateFileName(key.getTextElementAt(2), key.getTextElementAt(3),
		// key.getTextElementAt(4)));

	}

	private static final char SEP = (char) 1;
	/*
	 * @Override public final void cleanup(final Context context) throws
	 * IOException, InterruptedException { // mos.close(); }
	 */
}
