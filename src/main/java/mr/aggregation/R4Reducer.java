...
public class R4Reducer extends Reducer<TextMultiple, TextMultiple, BytesWritable, Text> {
  ...
  public final void reduce(final TextMultiple key, final Iterable<TextMultiple> values, final Context context)
    ...
    /*
     * guid => 0 itin_number => 1 trans_date => 2 num_transactions => 3 bkg_gbv => 4 bkg_room_nights => 5 omniture_gbv
     * => 6 omniture_room_nights => 7 gross_profit => 8
     */
    ...
    outStr
        .append(SEP)
        .append(key.getTextElementAt(3).toString())
        .append(SEP)
        .append(key.getTextElementAt(4).toString())
        .append(SEP)
  ...
  /*
   * @Override public final void cleanup(final Context context) throws IOException, InterruptedException { //
   * mos.close(); }
   */
}
