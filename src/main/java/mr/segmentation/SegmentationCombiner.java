...
public class SegmentationCombiner extends Reducer<TextMultiple, TextMultiple, TextMultiple, TextMultiple> {
    ...
    public final void reduce(final TextMultiple key, final Iterable<TextMultiple> values, final Context context) throws IOException,
        ...
        outVal.getTextElementAt(5).set(Long.toString(netOrders));
        outVal.getTextElementAt(6).set(Double.toString(netBkgGBV));
        outVal.getTextElementAt(7).set(Long.toString(netBkgRoomNights));
        outVal.getTextElementAt(8).set(Long.toString(numCancellations));
