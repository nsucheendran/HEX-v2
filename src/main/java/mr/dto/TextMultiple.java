...
public class TextMultiple implements WritableComparable<TextMultiple> {
	...
	public void stripeFlank(String[] texts, SegmentationSpec spec,
			String... appendables) {
		int i = 0;
	...
	public int compareTo(TextMultiple tm) {
		...
		return (texts.length == tm.texts.length) ? 0
				: (texts.length > tm.texts.length) ? 1 : -1;
	}
	...
	public void toStringBuilderSep(StringBuilder sb, char sep,
			int... excludePos) {
		boolean appended = false;
		for (int i = 0; i < texts.length; i++) {
			if (!containsArrayInt(excludePos, i)) {
