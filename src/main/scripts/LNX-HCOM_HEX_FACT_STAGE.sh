#!/bin/bash
#
# LNX-HCOM_HEX_FACT_STAGE.sh
# Wrapper to load hex fact staging data for HEX
#
# Usage:
#  LNX-HCOM_HEX_FACT_STAGE.sh
#
...
# user          date            comment
# ############# ############### ####################
# achadha       2013-10-21      Wrapper to incrementally load/reprocess Fact Staging 

set -m
export PLAT_HOME=/usr/local/edw/platform
export HWW_HOME=/usr/etl/HWW
SCRIPT_PATH=$HWW_HOME/hdp_hww_hex_etl/hql/FACT_STAGE
...
MESSAGE="LNX-HCOM_HEX_FACT_STAGE.sh failed: Previous script still running"
_ACQUIRE_LOCK $HWW_LOCK_NAME "$MESSAGE" 30

STAGE_NAME="HEX_FACT_STAGE: HEX Booking Fact Staging"
_LOG_START_STAGE "$STAGE_NAME"

PROCESS_NAME="ETL_HCOM_HEX_FACT_STAGE"
...
TABLE=`_READ_PROCESS_CONTEXT $PROCESS_ID "FACT_STAGE_TABLE"`
DB=`_READ_PROCESS_CONTEXT $PROCESS_ID "FACT_STAGE_DB"`
...
then
  ...
  _LOG "Incremental Booking Fact Staging data load (BOOKMARK=[$LAST_DT])"
  START_DT=`date --date="${LAST_DT} +1 days" '+%Y-%m-%d'`
  END_DT=$START_DT

  LOG_FILE_NAME="hdp_hex_fact_incremental_${START_DT}.log"
  MONTH=`date --date="${START_DT}" '+%Y-%m'`
