...
insert into table platdev.ETL_HCOM_HEX_ASSIGNMENT_HIT PARTITION(year, month)
  select temp.guid,
         ...
         temp.min_hit_data[13] as Platform_Type,
         temp.min_hit_data[14] as DAYS_UNTIL_STAY,
         temp.min_hit_data[15] as LENGTH_OF_STAY,
         temp.min_hit_data[16] as NUMBER_OF_ROOMS,
         temp.min_hit_data[17] as NUMBER_OF_ADULTS,
         temp.min_hit_data[18] as NUMBER_OF_CHILDREN,
         temp.min_hit_data[19] as CHILDREN_IN_SEARCH,
         year(temp.min_hit_data[0]) as year,
         month(temp.min_hit_data[0]) as month
    from 
					(select split(firstValueNSort(concat_ws("~~~", local_date, cast(gmt as string), cast(hit_data_id as string), cast(new_visitor_ind as string), 
					 page_assigned_entry_page_name, site_sectn_name,user_cntext_name,cast(browser_height as string),cast(browser_width as string),cast(brwsr_id as string),
                     case when c302 is null then 'Non Mobile' else 'Mobile' end, cast(destination_id as string), cast(property_destination_id as string), c277, 
                     cast(DAYS_UNTIL_STAY as string), cast(LENGTH_OF_STAY as string), cast(NUMBER_OF_ROOMS as string), cast(NUMBER_OF_ADULTS as string), 
                     cast(NUMBER_OF_CHILDREN as string), cast(CHILDREN_IN_SEARCH as string), c93), gmt, visit_page_number),"~~~") min_hit_data,
                                    cid,
                                    test_variant_code,
