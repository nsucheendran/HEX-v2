...
R4.omniture_bookmark_datetm='2012-10-31' (after run, update to {Omniture_bookmark_date})
-- bkg_xref's last transaction considered timestamp
R4.booking_bookmark_datetm='2012-10-31' (after run, update to min{Omniture_bookmark_date, Booking_bookmark_date})
...
gross_profit, guid, cid, itin_number, local_date, trans_date, variant_code, experiment_code, version_number, new_visitor_ind, page_assigned_entry_page_name, 
...
entry_page_name, substr(local_date, 1, 7) as year_month, source 
from 
(
	select count(1) as num_transactions, hits_by_report.variant_code, experiment_code,version_number, trans.guid, cid, hits_by_report.local_date as local_date, 
	trans.purchase_flag, trans.trans_date as trans_date, sum(bkg_gbv) as bkg_gbv, sum(bkg_room_nights) as bkg_room_nights, sum(gross_profit) as gross_profit, 
	new_visitor_ind, page_assigned_entry_page_name, site_sectn_name, user_cntext_name, browser_height, browser_width, brwsr_id, mobile_ind, destination_id, 
	property_destination_id, platform_type, days_until_stay, length_of_stay, number_of_rooms, number_of_adults, number_of_children, children_in_search,
	operating_system,all_mktg_seo,all_mktg_seo_direct, entry_page_name, source, trans.itin_number
	from
	(
		select guid, purchase_flag, local_date as trans_date, gmt, bkg_gbv, bkg_room_nights, gross_profit, source, itin_number 
		from
		hwwdev.ETL_HCOM_HEX_transactions 
		where year_month>='${R4.min_report_date_yrmonth}'
		and year_month<='${R4.max_trans_record_date_yr_month}'
		and local_date>='${R4.min_report_date}' 
		and 
		(
				(source='omniture' and local_date<='${R4.max_omniture_record_date}')
					or 
				(source='booking' and local_date<='${R4.max_booking_record_date}')
		)
	) trans
	right outer join 
	(
		...
		(
			...
			entry_page_name
			from 
			etldata.etl_hcom_hex_first_assignment_hit 
			where year_month>='${R4.min_report_date_yrmonth}' and year_month<='${R4.max_omniture_record_yr_month}'
			and local_date<='${R4.max_omniture_record_date}'
			and local_date>='${R4.min_report_date}' 
		) first_hits
		join
		(
			...
			from 
			platdev.HEX_REPORTING_REQUIREMENTS where 
			(last_updated_dt>='${hiveconf:R4.bookmark_datetm}') 
				or 
			(report_start_date<='${hiveconf:R4.bookmark_datetm}' and report_end_date>='${hiveconf:R4.bookmark_datetm}')
		) rep
	...
	where (trans.guid is null) 
				or 
	(
		trans.trans_date<=hits_by_report.trans_date and hits_by_report.gmt<=trans.gmt
			 and 
		(
			(
				hits_by_report.last_updated_dt>='${hiveconf:R4.bookmark_datetm}' 
					and 
				trans.trans_date>=hits_by_report.report_start_date
			)
				or 
			(
				(
					hits_by_report.last_updated_dt='' 
						or 
				...
				(
					(
						trans.source='booking' 
							and 
						trans.trans_date>'${R4.booking_bookmark_datetm}'
					) 
						or 
					(
						trans.source='omniture' 
							and 
						trans.trans_date>'${R4.omniture_bookmark_datetm}'
					)
				)
			)
		)
	)
	group by hits_by_report.variant_code, experiment_code, version_number, trans.guid, cid, trans.purchase_flag, hits_by_report.local_date, trans.trans_date, source, 
	new_visitor_ind, page_assigned_entry_page_name, site_sectn_name, user_cntext_name, browser_height, browser_width, brwsr_id, mobile_ind, destination_id, 
	...
	all_mktg_seo,all_mktg_seo_direct, entry_page_name, trans.itin_number
) rep_hit_trans;