...
insert overwrite table hwwdev.ETL_HCOM_HEX_METRICS partition(variant_code, experiment_code, version_number)
	...
	sum(nil_net_order_purchaser) as num_nil_net_order_purchasers,   
	...
	(
		...
		(
			...
			case when sum(num_transactions)>0 then min(trans_date) else null end as booking_date_for_active_itin, 
			...
			from 
			hwwdev.etl_hcom_hex_transactions_first_hits_aggregate_with_itinnum agg 
			join 
			(
				select variant_code,experiment_code,version_number, experiment_name, variant_name, status, experiment_test_id,
				case when report_end_date is null or report_end_date='' then '9999-99-99' else report_end_date end as report_end_date
				from 
				HWWDEV.HEX_REPORTING_REQUIREMENTS where 
				(last_updated_dt>='${hiveconf:R4.2.bookmark_datetm}') 
					or 
				(report_start_date<='${hiveconf:R4.2.bookmark_datetm}' and (report_end_date>='${hiveconf:R4.2.bookmark_datetm}' or report_end_date='' or report_end_date is null))
			) rep
